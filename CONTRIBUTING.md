...
Build order matters — `@agentboard/ao-core` must be built before the CLI, web, or plugins can run. `pnpm build` at the root handles this automatically.

### Running tests

```bash
pnpm test                                         # all packages
pnpm --filter @agentboard/ao-core test              # core only
pnpm --filter @agentboard/ao-core test -- --watch   # watch mode
...
pnpm --filter @agentboard/ao-web dev
```
...
`ao update` fast-forwards the local install repo, reinstalls dependencies, clean-rebuilds `@agentboard/ao-core`, `@agentboard/ao-cli`, and `@agentboard/ao-web`, refreshes the global launcher with `npm link`, and finishes with CLI smoke tests. Use `ao update --skip-smoke` when you only need the rebuild step, or `ao update --smoke-only` when validating an existing install.
...
```json
"@agentboard/ao-runtime-myplugin": "workspace:*"
```
...
```bash
pnpm --filter @agentboard/ao-runtime-myplugin build
pnpm --filter @agentboard/ao-runtime-myplugin test
```
