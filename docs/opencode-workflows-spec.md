...
- Core plugin registry built-ins must include `@agentboard/ao-plugin-agent-opencode` under slot `agent`.
...
- Unit/integration validation that should remain green for OpenCode workflows:
  - `@agentboard/ao-plugin-agent-opencode` tests.
  - `@agentboard/ao-core` tests: `session-manager.test.ts`, `plugin-registry.test.ts`.
  - `@agentboard/ao-cli` tests: `plugins.test.ts`, `start.test.ts`, `session.test.ts`, `send.test.ts`, `status.test.ts`.
  - `@agentboard/ao-integration-tests` with `test:integration` (includes `agent-opencode.integration.test.ts`, conditionally skipped tests where prerequisites are unavailable).
