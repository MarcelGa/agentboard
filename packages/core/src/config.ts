...
function resolveEnvOverrides(
  ...
  // Resolve path: relative paths are relative to the project directory
  const envFilePath = isAbsolute(project.envFile)
    ? project.envFile
    : join(project.path, project.envFile);
...
function applyProjectDefaults(config: OrchestratorConfig): OrchestratorConfig {
  for (const [id, project] of Object.entries(config.projects)) {
    ...
    // Resolve project-specific env overrides from envFile
